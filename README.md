...
Form-abandonment capture for Astro — and the complete lead-operations layer around it. Save the lead when a visitor types into your form and leaves without submitting, then recover, convert, and manage that lead from one self-hosted form backend with **zero external dependencies by default** — SQLite file storage plus your existing SMTP env vars — proven by **1,000+ unit tests** and a Playwright e2e suite.
...
Abandonment capture is the headline because no other open-source package offers it for Astro or static sites; the other 4 stages ship alongside it so a captured lead has somewhere to go.
...
2. Exit-intent, an external-link click, `beforeunload`, or a hidden tab triggers a POST of the staged fields to `/api/forms/abandon`.
...
7. Payments close the loop: create a quote from any entry, or share `/forms-pay?amount=200` — the server recomputes every total, and inbound webhooks are the sole source of payment truth.
...
Tag the form with `data-caf` and configure `coolForms()` — the injected client script stages typed values and POSTs them to `/api/forms/abandon` on exit-intent, an external-link click, `beforeunload`, or tab-hidden. The entry lands in SQLite with the visitor's journey trail and geolocation attached.
